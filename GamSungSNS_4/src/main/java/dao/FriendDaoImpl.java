...
public class FriendDaoImpl implements FriendDao {
	...
	public int deleteFriend(Friend friend) {
		int result = -1;
		String sql = "delete from friend where (from_user_no = ? and to_user_no = ?) or "
				+ "(from_user_no = ? and to_user_no = ?)";
	...
	public List<Map<String, Object>> selectFriendByToUserNo(int toUserNo) {
		...
		String sql = "select "
				...
				+ "requir_date requireDate, "
				+ "to_user_no toUserNo "
				+ "from friend "
				+ "where to_user_no = ? and from_user_no not like "
				+ "(select to_user_no from friend where from_user_no = ?)";
		
		//friend = jdbcTemp.queryForObject(sql, Friend.class,userNo, userNo);
		//List<Friend> list = jdbcTemp.query(sql, new BeanPropertyRowMapper<Friend>(Friend.class));
		List<Map<String, Object>> result = jdbcTemp.queryForList(sql,toUserNo,toUserNo);
		return result;
	}
	...
	public List<Friend> selectFriendByFromUserNo(int fromUserNo) {
		String sql = "select * from friend "
				+ "where from_user_no = ? and to_user_no "
				+ "not in (select from_user_no from friend where to_user_no = ?)";
		List<Friend> list = jdbcTemp.query(sql, 
				new BeanPropertyRowMapper<Friend>(Friend.class),fromUserNo, fromUserNo);
		return list;
	}
